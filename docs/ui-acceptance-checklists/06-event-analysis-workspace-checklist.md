# Event Analysis Workspace — Acceptance Checklist
...
- **Reference:** Approved Event Analysis Workspace mockup and corresponding engineering specification
...
## Event Analysis Workspace-specific acceptance criteria

- [ ] **PASS / FAIL:** `Saved events` is the active sidebar item for an opened saved analysis.
- [ ] **PASS / FAIL:** The breadcrumb identifies Saved events, event name, and Analysis.
- [ ] **PASS / FAIL:** The header displays event name, selected scenario, Duplicate scenario, and Compare scenarios controls.
- [ ] **PASS / FAIL:** Changing the scenario selector updates all metrics, summaries, checks, and input details for that scenario only.
- [ ] **PASS / FAIL:** Duplicating a scenario creates a distinct editable copy attached to the same event.
- [ ] **PASS / FAIL:** The scenario list shows status labels and the selected state clearly.
- [ ] **PASS / FAIL:** Event Snapshot and Input Summary display the specified fields and values.
- [ ] **PASS / FAIL:** `Edit inputs` opens the correct scenario in the event-input workflow.
- [ ] **PASS / FAIL:** Estimated revenue, total costs, estimated profit, profit margin, and break-even buyers are calculated correctly.
- [ ] **PASS / FAIL:** Expected buyers are rounded down and break-even buyers are rounded up where specified.
- [ ] **PASS / FAIL:** Profitability Summary recommendation state matches the configured profit rule and calculated result.
- [ ] **PASS / FAIL:** The amount above or below target is numerically correct and uses the correct semantic state.
- [ ] **PASS / FAIL:** Cost breakdown categories sum correctly and the visual representation is proportional to revenue.
- [ ] **PASS / FAIL:** Net profit and total-cost percentages reconcile with the displayed revenue.
- [ ] **PASS / FAIL:** Decision Checks show minimum target, amount above target, margin status, and break-even attendance correctly.
- [ ] **PASS / FAIL:** The sensitivity panel ranks the approved drivers using values derived from the current scenario.
- [ ] **PASS / FAIL:** `Save scenario`, `Duplicate`, and `Compare` execute the correct workflow and preserve data.
- [ ] **PASS / FAIL:** Auto-save status accurately reflects persistence state and does not claim success before storage completes.
- [ ] **PASS / FAIL:** Mobile surfaces the key metrics, recommendation, and Compare action before secondary details.
