# Scenario Comparison — Engineering Specification

## Document control

- **Product:** Food Truck Event Profit Calculator
- **Owner:** The Fixer’s Desk
- **Reference mockup:** `Scenario Comparison charcoal mockup`
- **Status:** Implementation blueprint
- **Source of truth order:** approved mockup → this specification → existing business logic

## Purpose

Compare multiple scenarios for the same event side by side and expose the assumptions that create each outcome.

**Dominant task:** Select the strongest scenario with acceptable risk.
...
- Compare 2–4 scenarios from one event.
- Default selection is the current scenario plus the most recently updated alternatives.
- Scenarios from different events cannot be compared on this screen.

## Header controls

- Event selector is read-only when entered from an event; it may switch events when entered from Saved Events.
- `Add scenario` opens a multi-select popover of unselected scenarios.
- `Keep selected scenario` marks the highlighted scenario as preferred and returns to analysis.
...
- The recommended/best scenario uses a copper border plus explicit badge or star; never rely on border alone.
- Positive financial values may use success color, but labels remain neutral.

## Assumption comparison table

Rows include Average ticket, Expected buyers, Service hours, Crew size, Travel & setup, Food cost percent, and Event conditions.

- Each scenario column shows the raw value and a compact relative bar.
- The Impact column explains directionality: higher is better, lower is better, capacity tradeoff, or conditions help.
- Bars are normalized per row, not across unlike units.
- Provide accessible text equivalent for every visual bar.

## Recommendation strip

- State which scenario is strongest and the reason using profit, margin, break-even, and risk.
- Display Best profit, Best margin, and Lowest break-even summary cards.
- Recommendation logic must use the same engine as Event Analysis Workspace.

## Interaction

- Clicking a scenario card selects it and updates the recommendation strip.
- `View assumptions` on compact layouts expands the comparison rows.
- `Back to analysis` returns to the scenario that was active before comparison unless the user kept another scenario.

## Responsive behavior

- Desktop: three cards in one row and full comparison table.
- Tablet: cards may wrap or use a horizontally scrollable snap region; table remains readable.
- Mobile: use scenario tabs or a carousel, show one summary card at a time, and provide a separate `View assumptions` comparison sheet. The final recommendation remains visible below.
