# Business Event Inputs Wizard — Engineering Specification

## Document control

- **Product:** Food Truck Event Profit Calculator
- **Owner:** The Fixer’s Desk
- **Reference mockup:** `Business Event Inputs Wizard charcoal mockup`
- **Status:** Implementation blueprint
- **Source of truth order:** approved mockup → this specification → existing business logic

## Purpose

Capture event-specific assumptions for one event and one scenario before profitability is calculated.

**Dominant task:** Create or update a complete event scenario.
...
- Event-specific inputs remain separate from business defaults.
- New scenarios begin from the current defaults snapshot, then store overrides.
- The active screen shown in the mockup is Revenue inputs, Step 2 of 5.

## Desktop layout

- Left event-section navigation: 292–312 px.
- Right form workspace: flexible.
- Stepper spans the top-right header and uses five labeled nodes.
- Sticky action row sits at the bottom of the form panel.

## Revenue method

Question: `How do you want to estimate revenue for this event?`

Options:

- `Use average ticket × buyers`
- `Use custom total sales`

Rules:

- Render only fields relevant to the selected method.
- For average-ticket mode, expected revenue is `average_ticket × expected_buyers`.
- Expected buyers may be directly entered or derived from attendance × conversion rate; derived buyers round down to a whole person.
- Custom-total-sales mode hides average ticket and buyer-derived revenue inputs, but attendance and operational fields may remain where they support other calculations.
- Switching methods must not leave both revenue values active in persistence.

## Core fields

- Scenario name — required, 1–80 characters.
- Event name — required, 1–120 characters.
- Event date — required valid local date.
- Estimated attendance — integer, >= 0.
- Average ticket — currency, >= 0, required in average-ticket mode.
- Buyer conversion rate — 0–100%, optional when expected buyers are manually provided.
- Expected buyers — integer, >= 0; derived values round down.
- Service hours — decimal > 0.

## Scenario snapshot

- Display Estimated revenue, Food cost method, Crew size, and Travel distance.
- Calculated values update after valid input changes.
- Include the persistent note that inputs apply only to this scenario and multiple scenarios may be saved for one event.

## Persistence and navigation

- Draft auto-saves locally with active step and validation state.
- `Save as draft` persists incomplete data and returns to Saved Events or Dashboard according to entry route.
- `Continue` validates current step.
- Final Review shows all inputs, default-derived values, overrides, and Edit links.
- Completing the wizard creates/updates the scenario and opens Event Analysis Workspace.

## Responsive behavior

- Tablet may retain two columns if each remains at least 280 px; otherwise place event sections above the form.
- Mobile stacks all fields, keeps the revenue selector visible, and uses a sticky full-width Continue button.
